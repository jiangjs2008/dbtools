 ...
 *
 */
public class SQLCipherCachedRowSetImpl implements CachedRowSet {

	private SQLCipherResultSetMetaData mrsr = null;
	 ...
	 * 构造函数
	 */
	public SQLCipherCachedRowSetImpl() {
	...
	public boolean next() throws SQLException {
		if (dataList != null && dataList.size() > 0 && curIdx < dataList.size() - 1) {
			curIdx ++;
			return true;
		} else {
			return false;
	...
	public String getString(int columnIndex) throws SQLException {
		ArrayList<Object> rowObj = dataList.get((int) curIdx);
		if (rowObj == null) {
			return null;
		} else {
			Object objValue = rowObj.get(columnIndex - 1);
			if (objValue == null) {
				return null;
			} else {
	...
	public boolean absolute(int row) throws SQLException {
		curIdx = row - 1;
		idxKeyObj = new BasicDBObject("_id", getString(1));
		return true;
	}
