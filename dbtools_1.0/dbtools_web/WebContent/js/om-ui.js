...
;(function($) {
	$.omWidget('om.omGrid', {
		...
		_create:function(){
			...
			this.hDiv = $('<div style="display:none"></div>').append('<div><table cellPadding="0" cellSpacing="0"></table></div>');
		...
		/**
		 * 自定义函数，表头和表格显示完后重新调整宽度
		 */
		__fixWidth : function() {
			//datagrid头部 table 的第一个tr 的td们，即columns的集合
			var headerTds = $(".bDiv table thead tr:first-child").children();

			//合计宽度，用来为datagrid头部和主体设置宽度
			var totalWidth = 0;
			//循环设置宽度
			for (var i=0,len=headerTds.length; i<len; i++) {

		//	}
		//	bodyTds.each(function (i, obj) {
				var headerTd = $(headerTds.get(i));
				var headerTdWidth = headerTd.width() + 20;
		...
		_buildTableHead:function(){
			...
			for (var i=0,len=cms.length;i<len;i++) {
				...
				var thCell=$('<div></div>').html(cm.header).css({'text-align':cmAlign});
				cm.wrap && thCell.addClass('wrap');
				var th=$('<th style="background: #e6e699;padding 15px;border:1px"></th>').attr('axis', 'col' + i).addClass('col' + i).append(thCell);
				if(cm.name) {
