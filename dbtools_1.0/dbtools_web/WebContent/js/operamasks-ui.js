...
$.fn.position = function( options ) {
	...
	return this.each(function() {
		var elem = $( this ),
			elemWidth = elem.width(),  // 注：这里原来用的是outerWidth()和outerHeight()
...
;(function($) {
	$.omWidget('om.omGrid', {
		...
		_create:function(){
			...
			if(!options.autoColModel && !$.isArray(this._getColModel())){
		...
		_init:function(){
			...
			if(!ops.autoColModel && !$.isArray(this._getColModel())){
				return; //如果colModel没设置或值不对，什么也不做
			}
			ops._haveColData = false;
		...
		_fixHeaderWidth:function(ops) {
			// 列宽由内容长度决定，自适应宽度
			//datagrid头部 table 的第一个tr 的th们，即columns的集合
			var headerTds = $(".hDivBox table tr:first-child").children();
			...
			bodyTds.each(function (i, obj) {
			  ...
			  if (headerTdWidth > bodyTdWidth) {
			      headerTdWidth = headerTdWidth;
			  } else {
			      headerTdWidth = bodyTdWidth;
			  }
			  if (headerTdWidth > 800) {
			  	headerTdWidth = 800;
			  }
				cellWidth[i] = headerTdWidth ;
				headerTd.children().first().css("width", cellWidth[i]);
			});

			// 以上只重置了第一行的宽度，以下检查列宽超过800的列
			var bodyTrs = $(".bDiv table tr");
			bodyTrs.each(function (i, obj) {
				$(obj).children().each(function (j, obj2) {
					$(obj2).children().first().css("width", cellWidth[j]);
				});
			});
		},
		_fixTableWidth:function(ops) {
			// 指定列宽(初始化grid时指定)，固定宽度
			// TODO-- 暂未考虑列要自动扩充
		...
		_buildTableHead:function(){
			...
			if (op.autoColModel && !op._haveColData) {
				return;
			}
			//渲染序号列
			if(op.showIndex){
			...
			for (var i=0,len=cms.length;i<len;i++) {
				...
				var thCell = null;
				if (op.autoColModel) {
					thCell = $('<div></div>').html(cm.header).css({'text-align':cmAlign});
				} else {
					thCell = $('<div></div>').html(cm.header).css({'text-align':cmAlign, 'width':cmWidth});
				}
		...
		_populate : function() { // get latest data
			...
			$.ajax({
				...
				success : function(data,textStatus,request) {
					...
					// 重新校正各列宽度
					if (op.autoColModel) {
						self._fixHeaderWidth(op);
					} else {
						self._fixTableWidth(op);
					}
		...
		_renderDatas:function(){
			var self=this,
				...
				tdTmp = "<td align='$' abbr='$' class='$'><div align='$' class='innerCol $' >$</div></td>",//td模板
				//headerWidth = [],
				bodyContent = [],
				cols,
				j;
			...
			$.each(rows,function(i, rowData) {
				...
				$(gridHeaderCols).each(function(index){
					...
					cols = [this.align , this.abbr , axis , this.align , wrap?'wrap':'',  html];
					j=0;
	...
	$.omWidget('om.omCombo', {
		...
		_showDropList:function(){
			...
			dropListContainer.css({
				'left': inputPos.left,
				'top': inputPos.top + span.outerHeight(true)
			});
			dropList.show();
	...
	$.omWidget('om.omMenu', {
		...
		show : function(triggerEle){
			...
			if( options.contextMenu ){
				top = triggerEle.originalEvent.pageY;
				left = triggerEle.originalEvent.pageX;
